...
namespace Sistema
{
    public partial class anular_prestamo : Form
    {
        ...
        public void obtenerfacturas()
        {
            ...
            {
                conexion.Open();
                OleDbCommand comando = new OleDbCommand("SELECT DISTINCT numero_prestamo FROM prestamo where fecha_anulacion = '00' ORDER BY numero_prestamo ASC", conexion);
        ...
        public void actualizar()
        {
            ...
            dt.Clear();
            dtin.Clear();
            OleDbDataAdapter data = new OleDbDataAdapter("SELECT *FROM prestamo where fecha_anulacion = '00' and numero_prestamo='" + comboBox1.Text+"'", conexion);
            data.Fill(dt);
        ...
        {
            conexion.Open();
            OleDbDataAdapter data = new OleDbDataAdapter("SELECT id, tienda1 FROM inventario WHERE codigo='" + codigo+ "' AND marca= '"+marca+"' AND grupo ='"+grupo+"'", conexion);
            data.Fill(dtin);
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            if (comboBox1.Text == "")
            {
                MessageBox.Show("Elija un numero de Prestamo", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
            else
            {
                ...
                try
                {
                    ...
                    foreach (DataGridViewRow factura in dataGridView1.Rows)
                    {
                        ...
                        string update = "UPDATE prestamo set fecha_anulacion = '" + fecha + "' WHERE numero_prestamo= '" + comboBox1.Text + "'";
                    ...
                    foreach (DataGridViewRow inven in dataGridView2.Rows)
                    {
                        ...
                        string update = "UPDATE inventario set tienda1 = '" + suma + "' WHERE id= " + id + "";
                        OleDbCommand comando55 = new OleDbCommand(update, conexion);
                        comando55.ExecuteNonQuery();
                        contador++;
                    }
                    conexion.Close();
                    MessageBox.Show("Ha anulado el Prestamo con exito", "Éxito", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    this.Close();
                }
                catch (Exception ex)
                {
