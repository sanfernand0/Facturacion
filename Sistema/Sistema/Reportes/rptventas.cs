...
namespace Sistema
{
    public partial class rptventas : Form
    {
        ...
        public void obtenerFacturas()
        {
            ...
            OleDbCommand comando = new OleDbCommand("SELECT  numero_factura FROM ventas ", conexion);
        ...
        private void printDocument1_PrintPage(object sender, System.Drawing.Printing.PrintPageEventArgs e)
        {
            try
            {
                ...
                e.Graphics.DrawString("Valor Lps", FuenteEncabezados, Brocha, 730, 146);
                Y = Convert.ToInt32(Y + Fuente.GetHeight());
                ...
                while (item < dataGridView1.Rows.Count)
                {
                    ...
                    {
                        e.Graphics.DrawString(Convert.ToDateTime(dataGridView1.Rows[fila].Cells["Fecha"].Value).ToShortDateString(), Fuente, Brocha, 23, Y);
                        ...
                        e.Graphics.DrawString(Convert.ToDecimal(dataGridView1.Rows[fila].Cells["Total"].Value.ToString()).ToString("N2"), Fuente, Brocha, 730, Y);
                ...
                e.Graphics.DrawRectangle(Pens.Black, 20, Y, 785, 80);
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            ...
            string c = @"SELECT fecha AS Fecha, numero_factura AS Factura, cliente AS Cliente, vendedor AS Vendedor, 
                          tipo AS TipoVenta,  sum(cantidad*precio) AS Total, isv as ISV, ((cantidad*precio)*isv) as TotalIsv 
                       FROM ventas";
            string order = " order by fecha desc, numero_factura desc";
            string order2 = " order by fecha";
            string group = " group by numero_factura, fecha, cliente, vendedor, tipo, isv, ((cantidad*precio)*isv)";
            string group2 = " group by numero_prestamo, fecha, cliente, vendedor, tipo";


            string c2 = c + " WHERE numero_factura='" + textBox5.Text + "' AND fecha_anulacion = '00'" +  group + order;
            string c7 = c  + group + order;


            string ccc = c + " WHERE fecha_anulacion = '00' AND numero_factura='" + textBox5.Text + "' AND fecha between #" + dateTimePicker1.Value.ToShortDateString() + "# AND #" + dateTimePicker2.Value.ToShortDateString() + "#" + group + order;
            string cc = c + " WHERE fecha_anulacion = '00'AND fecha between Format(#" + dateTimePicker1.Value.ToShortDateString() +"#, 'm/d/yyyy')" + " AND Format(#" + dateTimePicker2.Value.ToShortDateString() + "#, 'm/d/yyyy')"  + group + order;



            string v = @"SELECT fecha AS Fecha, numero_prestamo AS Factura, cliente AS Cliente, vendedor AS Vendedor, tipo AS TipoVenta,  sum(cantidad*precio) AS Total FROM prestamo";
            string v2 = v + " WHERE fecha_anulacion = '00' AND numero_prestamo='" + textBox5.Text + "'" + group2 + order2;
            string vvv = v + " WHERE fecha_anulacion = '00' AND numero_prestamo='" + textBox5.Text + "' AND fecha between Format(#" + dateTimePicker1.Value.ToShortDateString() + "#, 'm/d/yyyy')" + " AND Format(#" + dateTimePicker2.Value.ToShortDateString() + "#, 'm/d/yyyy')" + group2 + order2;
            string vvvv = v + " WHERE fecha_anulacion = '00' AND fecha between Format(#" + dateTimePicker1.Value.ToShortDateString() + "#, 'm/d/yyyy')" + " AND Format(#" + dateTimePicker2.Value.ToShortDateString() + "#, 'm/d/yyyy')" + group2 + order2;
            string vv = v + group2 + order2;
