...
namespace Sistema
{
    public partial class anular_factura : Form
    {
        ...
        public void obtenerfacturas()
        {
            ...
            {
                conexion.Open();
                OleDbCommand comando = new OleDbCommand("SELECT DISTINCT numero_factura FROM ventas WHERE fecha_anulacion = '00' ORDER BY numero_factura ASC", conexion);
        ...
        public void actualizar()
        {
            ...
            dt.Clear();
            dtin.Clear();
            OleDbDataAdapter data = new OleDbDataAdapter("SELECT *FROM ventas WHERE fecha_anulacion = '00' and  numero_factura ='" + comboBox1.Text+"'", conexion);
            data.Fill(dt);
            ...
            else
            {
                try
                {
                    ...
                    foreach (DataGridViewRow factura in dataGridView1.Rows)
                    {
                        ...
                        string update = "UPDATE ventas set fecha_anulacion = '" + fecha + "' WHERE numero_factura= '" + comboBox1.Text + "'";
                    ...
                    conexion.Close();
                    MessageBox.Show("Ha anulado la factura con exito", "Éxito", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    this.Close();
                }
                catch (Exception ex)
                {
